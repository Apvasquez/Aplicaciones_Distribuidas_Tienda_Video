                    ...
                    <div x-show="open">
                        <div class="font-mono text-slate-900 border m-2 p-2 rounded shadow-md text-center ">
                            ...
                            <label class="block mx-auto mb-2 text-center">
                                <span for="descripcion" class="mb-2 font-bold form-label">Descripción</label>
                            <input wire:model="descripcion" type="text" placeholder="Ingrese La descripción "
                                class="w-1/2 rounded form-input" required>
                            ...
                            <label class="block mx-auto mb-2 text-center">
                                <span for="lugar" class="mb-2 font-bold form-label">Lugar</label>
                            <input wire:model="lugar" type="text" placeholder="Ingrese el lugar del proveedor "
                                class="w-1/2 rounded form-input" required>
                            @if ($accion == 'store')
                        ...
                        <div class="w-full mx-auto overflow-hidden bg-white rounded-lg shadow min-w-max-content">
                            <table class="w-full overflow-hidden bg-white rounded-lg shadow ">
                                ...
                                <tbody class="w-full mx-auto divide-y divide-gray-300">
                                    @foreach ($proveedor as $corres)
                                        <tr class="text-xs text-justify text-gray-500">
                                            <td class="px-4 ">{{ $corres->id }}</td>
                                            <td class="px-4 ">{{ $corres->nombre }}</td>
                                            <td class="px-4 ">{{ $corres->descripcion }}</td>
                                            <td class="px-4 ">{{ $corres->lugar }}</td>
